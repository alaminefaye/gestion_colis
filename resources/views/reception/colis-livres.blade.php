...
<div class="row">
  <div class="col-12">
    ...
    <div class="card mb-4" style="background: linear-gradient(135deg, #28a745 0%, #20c997 100%);">
      <div class="card-body text-white">
        <div class="row align-items-center">
          <div class="col-md-8">
            <h4 class="text-white mb-2">
              <i class="ti ti-truck me-2"></i>Colis Livrés
            </h4>
            <p class="mb-0 opacity-75">
              📊 <span id="total-count">{{ $colis->total() }}</span> colis livrés au total
            </p>
          </div>
    ...
    <div class="card mb-4">
      <div class="card-header">
        <div class="row align-items-center">
          ...
          <div class="col-auto">
            <button class="btn btn-outline-success btn-sm" type="button" data-bs-toggle="collapse" data-bs-target="#searchFilters">
      ...
      <div class="card-body">
        ...
        <div class="row mb-3">
          ...
          <div class="col-md-3">
            <div class="form-group">
              <label class="form-label">Livré par</label>
              <select id="livreurFilter" class="form-select">
                <option value="">Tous les livreurs</option>
                @foreach($colis->unique('livreurLivraison.nom')->whereNotNull('livreurLivraison') as $item)
                  @if($item->livreurLivraison)
                    <option value="{{ $item->livreurLivraison->nom_complet }}">{{ $item->livreurLivraison->nom_complet }}</option>
                  @endif
                @endforeach
              </select>
            </div>
        ...
        <div class="collapse" id="searchFilters">
          <div class="row">
            ...
            <div class="col-md-3">
              <div class="form-group">
                <label class="form-label">Date livraison (du)</label>
                <input type="date" id="dateFrom" class="form-control">
              </div>
            </div>
            <div class="col-md-3">
              <div class="form-group">
                <label class="form-label">Date livraison (au)</label>
    ...
    <div class="card">
      <div class="card-header">
        <h5 class="mb-0">📋 Liste des Colis Livrés</h5>
      </div>
      <div class="card-body">
        @if($colis->count() > 0)
            ...
            <table class="table table-hover" id="colisTable">
              ...
              <tbody>
                ...
                <tr class="colis-row" 
                    ...
                    data-date="{{ $item->livre_le?->format('Y-m-d') }}"
                    data-livreur="{{ $item->livreurLivraison?->nom_complet }}">
                  <td>
                  ...
                  <td>
                    <div>
                      @if($item->livre_le)
                        <strong>{{ $item->livre_le->format('d/m/Y') }}</strong><br>
                        <small class="text-muted">{{ $item->livre_le->format('H:i') }}</small>
                      @else
                    ...
                    @if($item->livreurLivraison)
                      <div class="d-flex align-items-center">
                        <div class="avatar avatar-sm bg-success text-white rounded-circle me-2">
                          {{ substr($item->livreurLivraison->nom, 0, 1) }}{{ substr($item->livreurLivraison->prenom, 0, 1) }}
                        </div>
                        <div>
                          <small><strong>{{ $item->livreurLivraison->nom_complet }}</strong></small><br>
                          <small class="text-muted">{{ $item->livreurLivraison->telephone }}</small>
                        </div>
                      </div>
                    @else
                  ...
                  <td>
                    <div class="d-flex gap-1">
                      ...
                      @if($item->notes_livraison)
                        <button class="btn btn-outline-info btn-sm" title="Voir notes de livraison" 
                                onclick="showNotes('{{ addslashes($item->notes_livraison) }}')">
        ...
        @else
          <div class="text-center py-5">
            <div class="mb-3">
              <i class="ti ti-truck-off" style="font-size: 4rem; color: #dee2e6;"></i>
            </div>
            <h5 class="text-muted mb-3">Aucun colis livré</h5>
            <p class="text-muted mb-4">
              Aucun colis n'a encore été livré avec les critères de recherche actuels.
            </p>
            <div class="d-flex gap-2 justify-content-center">
              <a href="{{ route('application.ecom-product-list') }}" class="btn btn-primary">
                <i class="ti ti-package me-2"></i>Voir Tous les Colis
              </a>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  livreurFilter.addEventListener('change', filterAndSort);
  ...
  function filterAndSort() {
    ...
    const livreurFilterValue = livreurFilter.value;
    ...
    rows.forEach(row => {
      ...
      const matchesLivreur = livreurFilterValue === '' || livreur === livreurFilterValue;
      ...
      if (matchesSearch && matchesLivreur && matchesDestination && matchesType && matchesDate && matchesMontant) {
        row.style.display = '';
